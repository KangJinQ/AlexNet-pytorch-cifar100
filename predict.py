...
model_path = "./model/cifar100_0.56.pkl"
...
datas, targets = next(iter(test_dataloader))
...
for i in range(datas.shape[0]):
    predict = model(datas[i]).detach()
    yh = torch.argmax(predict)
    count += 1 if yh == targets[i] else 0
    y_hat.append(yh.to("cpu").numpy())

acc = count / 10
print("the accuracy of prediction is {}".format(acc))

datas = datas.to("cpu")
plt.subplots(5, 2)
for num in range(datas.shape[0]):
    plt.subplot(2, 5, num+1)
    plt.imshow(datas[num].permute(1, 2, 0))
    # plt.title(fineLableToCoraseLabelDict[target[num].item()])
    plt.title(fineLabelNameDict[targets[num].item()]+"\np:{}".format(fineLabelNameDict[y_hat[num].item()]))
plt.show()
